...
function useDebouncedCinemaMutation(delay = 500) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      console.log(
        "Cinema selections updated successfully:",
        variables.requestBody,
      )
      queryClient.setQueryData(
        ["user", "cinema_selections"],
        variables.requestBody,
      )
...
const Filters = ({ selectedDays, handleDaysChange }: FiltersProps) => {
  ...
  const handleToggleCity = (cityId: number) => {
    ...
    const allSelected = cityCinemas.every((cinema) =>
      selectedCinemas.includes(cinema.id),
    )

    setSelectedCinemas((prev) => {
      const next = allSelected
        ? selectedCinemas.filter(
            (id) => !cityCinemas.some((cinema) => cinema.id === id),
          )
      ...
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            ...
            <Dialog.Body>
              <Grid templateColumns={"40% 40%"} gap={20}>
                <Box>
                  {Object.values(groupedCinemas).map(
                    ({ city, cinemasForCity }) => (
                      <CityCinemas
                        key={city.id}
                        city={city}
