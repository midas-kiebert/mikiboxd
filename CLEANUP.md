...
- [x] `Feed/FeedPageShell.tsx` — The chrome every feed page shares: layout, toolbar, rail, presets and the loading/empty/paging states. Rows are the caller's, which is what lets the home feed swap between screenings and films without swapping anything around them
...
- [x] `Feed/CinemaPresets.tsx` — Named cinema selections at the top of the Cinemas section. The favourite is "my cinemas" and the default is the synthetic "All cinemas" row with no database row behind it; both are told apart by flag and never by name, because both can be renamed
...
- [x] `Showtimes/SeatFloorPlan.tsx` — The room, drawn. All the geometry is `shared/showtimes/seat-floor-plan-layout` (the same code the app runs); this measures its container with a ResizeObserver, hands over the two numbers, and paints what comes back. Takes the availability reading's `checked_at` so the map and the seat count beside it cannot disagree
- [x] `Showtimes/SeatAvailabilitySection.tsx` — How full the room is, a one-shot "check how many seats are left", and the sold-out watch. Renders nothing when `trackable === false`, which means "this cinema never tells us" rather than "not looked yet" — gated on `=== false` so a missing flag on an older payload does not hide it
...
- [x] `Showtimes/ShowtimeInvites.tsx` — Invite, uninvite and nudge friends for one showtime, inside the detail panel. Search appears past eight friends, because a flat list stops being scannable well below the number anyone has. Hidden entirely for guests rather than gated: a section that could only ever be empty has nothing to discover
...
- [x] `showtimes/seat-availability-level.ts` — How a busyness level is worded, plus the seat-count and "checked 5m ago" formatting. Split out of the app's module so the website describes the same level the same way; icons and theme colours stay behind in mobile, which re-exports the rest
