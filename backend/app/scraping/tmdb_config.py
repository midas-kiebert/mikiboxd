...
# the exact-year tie-break. 16 = the own-title tie-break (`_prefer_own_title_match`),
# which only turns ties that used to be rejected into matches — but those
# rejections are cached too, and would otherwise never be retried. Admin
# corrections survive a bump: they are matched on the normalized title, not the
# payload hash (see `_get_manual_override_for_title`).
TMDB_LOOKUP_PAYLOAD_VERSION = 16
