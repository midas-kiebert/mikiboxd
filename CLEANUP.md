...
- [x] `filters/PresetButton.tsx` — One saved-preset button, owning its own tap animation: a soft tint in the green accent trio plus a short scale pop, settling back to its resting colours. Per button rather than per row on purpose — two presets tapped in quick succession then light and fade on their own timelines instead of the first being cut off mid-fade by the second. Colours are interpolated on the JS driver and the scale runs natively, hence the two animated values. Replaced an opaque accent overlay with a check in it, which read as a solid slab and could only ever animate one button at a time
- [x] `filters/filter-change-animation.ts` — How the filter chips react to a change and how they move while they do. Two separate things on purpose: *motion* (entering, crumpling away, sliding to a new position) runs for every change including a hand edit, because a chip that snaps from one place to another is what makes a row hard to follow; *colour* runs only for a preset apply, the change nobody watched happen. Only an arriving chip gets anything of its own (a green tint, mixed part-way to the accent since a full-strength card fill shouts on something chip-sized) — a chip whose value was rewritten is already in place and already moving, its width changing and its label sliding across, and both a tint and a swell on top of that were worse than nothing: the swell fought the resize whenever the new value was the shorter one. Everything animates one view's own transform and opacity — a version of the exit that scattered dust motes across sibling views, driven by a shared clock the layout animation started, did not work. The cinema pill carries the same layout transition: its label is a count one moment and a preset's name the next, so it resizes constantly, and tweening its width against chips that slide by the same amount over the same duration is what keeps them off each other. Exits are sequenced ahead of moves and entrances (`CHIP_LAYOUT_AFTER_EXIT`, `chipEnteringAfterExit`): Reanimated drops a removed chip from the layout the moment it unmounts while still drawing it in place, so without the delay the survivors slide straight through it — visible as overlapping chips whenever a preset removed several at once. Reanimated rather than RN's `Animated`: a chip leaving has to outlive its filter and its neighbours have to slide, neither of which `Animated` can do. `CHANGE_HIGHLIGHT_MS` is exported because cutting the change prop early snaps the tint off mid-fade
- [x] `filters/preset-apply-signal.ts` — "A saved preset was just applied", announced to whoever shows the result. The chips cannot see where a prop change came from, so the press says so directly — a user removing a chip by hand is not animated, since flagging their own edit back at them explains nothing. A counter, not a boolean: two applies in a row are two events
