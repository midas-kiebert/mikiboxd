...
const UserCard = ({ user }: UserCardProps) => {
  ...
  const removeFriendMutation = useMutation({
    mutationFn: (data: FriendsRemoveFriendData) =>
      FriendsService.removeFriend(data),
    onSuccess: () => {
  ...
  const sendFriendRequestMutation = useMutation({
    mutationFn: (data: FriendsSendFriendRequestData) =>
  ...
  const acceptFriendRequestMutation = useMutation({
    mutationFn: (data: FriendsAcceptFriendRequestData) =>
      FriendsService.acceptFriendRequest(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users"] })
    },
    onError: (error) => {
  ...
  const cancelFriendRequestMutation = useMutation({
    mutationFn: (data: FriendsCancelFriendRequestData) =>
      FriendsService.cancelFriendRequest(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users"] })
    },
    onError: (error) => {
  ...
  if (user.is_friend) {
    ...
    button = (
      <IconButton
        ...
        size={{ base: "2xs", md: "xs" }}
        onClick={() =>
          declineFriendRequestMutation.mutate({ senderId: user.id })
        }
      >
        <Icon as={LiaTimesSolid} boxSize={5} />
      </IconButton>
    )
    secondButton = (
      <IconButton
        ...
        size={{ base: "2xs", md: "xs" }}
        onClick={() =>
          acceptFriendRequestMutation.mutate({ senderId: user.id })
        }
      >
        <Icon as={IoIosCheckmark} boxSize={7} />
      </IconButton>
    )
    ...
    button = (
      <IconButton
        ...
        size={{ base: "2xs", md: "xs" }}
        onClick={() =>
          cancelFriendRequestMutation.mutate({ receiverId: user.id })
        }
      >
        <Icon as={LiaTimesSolid} boxSize={5} />
      </IconButton>
    ...
    button = (
      <IconButton
        ...
        size={{ base: "2xs", md: "xs" }}
        onClick={() =>
          sendFriendRequestMutation.mutate({ receiverId: user.id })
        }
      >
        <Icon as={LiaPlusSolid} boxSize={6} />
      </IconButton>
    )
    secondButton = null
  }
