...
export type ActivityFriendTally = {
  user: UserPublic
  going: number
  interested: number
}
...
export type ActivitySummaryPublic = {
  days: Array<ActivityDayCount>
  total: number
  next_plan: ShowtimePublic | null
  plan_count: number
  invites: Array<ShowtimePublic>
  invite_count: number
  friends: Array<ActivityFriendTally>
}
...
export type CinemaPublic = {
  ...
  city: CityPublic
  /**
   * Other names this cinema is known by (Cineville venue names, former display names). Matched when ingesting showtimes and when searching by cinema.
   */
 ...
 */
export type FeedOverviewPublic = {
  sections: Array<FeedOverviewSection>
}

export type FeedOverviewSection = {
  kind: FeedOverviewSectionKind
  showtimes: Array<ShowtimePublic>
}
 ...
 */
export type FeedOverviewSectionKind =
  | "invited"
  | "selling_fast"
  | "custom"
  | "plans"
...
export type MeCountMyShowtimesData = {
  ...
  /**
   * Only showtimes these friends are going to or interested in (as far as they let the viewer see)
   */
  ...
  hideWatched?: boolean
  /**
   * With selected_statuses, match only the viewer's own selections — their agenda
   */
  onlyYou?: boolean
  query?: string | null
  /**
...
export type MeGetFeedOverviewData = {
  /**
   * Skip the viewer's usual-cinemas default; this feed is already scoped to everyone (or everyone but the viewer)
   */
  allCinemas?: boolean
  ...
  hideWatched?: boolean
  /**
   * Whether the filter parameters describe the viewer's own list
   */
  includeCustom?: boolean
  /**
   * False where the page is the viewer's agenda already
   */
  ...
  onlyYou?: boolean
  /**
   * The feed ignores the usual cinemas; so does the watchlist list
   */
  overviewAllCinemas?: boolean
  /**
   * The feed's cinema selection, for the watchlist list; none means the account's usual cinemas
   */
  overviewCinemaIds?: Array<number> | null
  /**
   * The feed's language choice; the friends and watchlist lists follow it
   */
  overviewLanguages?: Array<Language> | null
  query?: string | null
  /**
   * Maximum movie runtime in minutes
   */
...
export type MeGetFeedOverviewResponse = FeedOverviewPublic

export type MeGetActivitySummaryData = {
  mode?: ActivityMode
  /**
   * The list's own snapshot, so the counts match its rows
   */
  snapshotTime?: string | null
}
