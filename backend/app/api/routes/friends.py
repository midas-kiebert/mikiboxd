    ...
    *, session: SessionDep, current_user: CurrentUser, receiver_id: uuid.UUID
    ...
    try:
        crud.send_friend_request(
            session=session, sender_id=current_user.id, receiver_id=receiver_id
        )
    ...
    *, session: SessionDep, current_user: CurrentUser, sender_id: uuid.UUID
    ...
    try:
        crud.accept_friend_request(
            session=session, receiver_id=current_user.id, sender_id=sender_id
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get("/")
def get_friends(*, session: SessionDep, current_user: CurrentUser) -> UsersPublic:
    """
...
def get_received_friend_requests(
    *, session: SessionDep, current_user: CurrentUser
    ...
    """
    requests = crud.get_received_friend_requests(
