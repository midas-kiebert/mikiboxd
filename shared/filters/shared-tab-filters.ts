import type { CinemaPresetPublic, GoingStatus } from "../client";

import { formatDayPillLabel } from "./day-filter-utils";
import { formatRuntimePillLabel } from "./runtime-range-utils";
import { formatTimePillLabel } from "./time-range-utils";
...
/**
 * Only the two accent trios this module actually reads, rather than the whole
 * theme: structural, so the mobile `Colors.light`/`Colors.dark` objects satisfy
 * it as-is and the website can pass its own tokens without owning a copy of the
 * app's theme shape.
 */
type ThemeColors = {
  green: { primary: string; secondary: string };
  orange: { primary: string; secondary: string };
};
