...
- [x] `features/showtimes/useAgendaFeeds.ts` — Your agenda and a friend's, in the same shape as the main feed. Each passes only the filters its endpoint accepts, so no control on screen silently does nothing
...
- [x] `UserSettings/EmailVerificationNotice.tsx` — "Confirm your email", with a resend, shown only while it is unconfirmed. Verification is a soft gate (it blocks the digest, not the account), so it is a line on the profile tab rather than anything that stops you using the page
- [x] `UserSettings/WatchlistDigest.tsx` — Digest sources: one standing question each (a list, a set of cinemas, a cadence). Frequency labels come from the backend so the two clients and the emails cannot describe the same cadence differently
- [x] `Friends/FriendStatusSharingToggle.tsx` — Whether one friend can see your plans. Sits alongside per-showtime visibility: the mode decides which group can see a selection, this decides whether someone is in that group at all. Reads `shares_status` as on-unless-false so an older payload without the field is not treated as opted out
- [x] `UserSettings/LetterboxdLists.tsx` — Add, re-sync and remove Letterboxd lists. The feed could already filter by a list and there was no way to add one, so the only lists a web user could reach were the curated ones. Curated lists show but cannot be removed: they belong to everyone
...
- [x] `Notifications/NotificationsPage.tsx` — Activity: invites, friend requests, friend activity and the seat alerts nobody caused, in one list. Row wording comes from `shared/notifications/feed-copy`, the same function the app's row uses. Keeps the `/pings` path because the app links to it
- [x] `Feed/FeedPresets.tsx` — Saved filter presets, in the filter rail: apply, save, favourite, delete. The model is entirely shared with the app, including the part that is easy to get wrong -- a preset *controls* every dimension except the ones it was saved as leaving alone, so applying one clears hand-set filters on purpose. Squared buttons that never render as selected, with the favourite marked by a filled star rather than a fill on the button — on the rail a rounded, tinted preset would be indistinguishable from the state pills a few pixels below it
- [x] `Feed/ShowtimeFeedPage.tsx` — Every showtime feed on the website, rendered once: home, a cinema's programme, your agenda, a friend's agenda. They differ only in where their rows come from and what they say when empty, so a change to how a feed looks happens here instead of four times with three forgotten
- [x] `Feed/FeedPageShell.tsx` — The chrome every feed page shares: layout, toolbar, rail, presets and the loading/empty/paging states. Rows are the caller's, which is what lets the home feed swap between screenings and films without swapping anything around them. It also places the feed's two page-level controls, which is a question about the window and not about the feed: the search field goes to the navigation (falling back to a bar above the list where there is no nav, or no room for one on a phone) and "Clear filters" goes to the filter rail, beside the filters it clears. On the home and films feeds both land elsewhere, so nothing is rendered above the list at all
- [x] `Feed/MovieFeedPage.tsx` — A feed of films rather than screenings: the films tab, and the home feed with "one row per film" on. No detail panel — a film's detail is its own page
- [x] `Feed/CinemaPresets.tsx` — Named cinema selections at the top of the Cinemas section. The favourite is "my cinemas" and the default is the synthetic "All cinemas" row with no database row behind it; both are told apart by flag and never by name, because both can be renamed. A row no longer lights up when the current selection matches it — it sits beside the cinema chips, which *are* selections, so the same rule the saved presets follow applies here
- [x] `Feed/FeedFilterRail.tsx` — The filter rail beside the feed: day tokens, time of day, length, language, watchlist/watched, city-grouped cinemas and Letterboxd lists (each list a mutually exclusive Only/Hide pair). The short dimensions are open on arrival and one click away, where the app costs six actions behind a button, a sheet and a section header; the two unbounded lists (cinemas, Letterboxd) start collapsed with a summary, because leaving them open pushes the card past the viewport and costs the rail its pinning. Presets, group-by-film and the friend-status filter sit above the dimensions — one sets all of them at once, the other two decide what a row even is and whose marks count. Both of those are segmented controls, matching the app down to its "Showtimes"/"Movies" and "Any"/"Interested"/"Going" wording; the friend filter is hidden for a guest, whose section could only ever be empty. Cinemas are chips rather than a checkbox column — what the app's own picker uses, legible on the tint where a checkbox control is not, and what the card's width is for. The rail paints its own card (brand tint, squared corners, no outline) rather than taking `FeedLayout`'s, because it is the one panel with a colour of its own and its dividers run the full width. Adding a dimension is a section in this file — the state, URL spelling and API mapping are already in `feed-params.ts`
- [x] `Feed/FilterRailControls.tsx` — The rail's vocabulary: collapsible section, pill, sub-heading. Every one is the app's own control rendered for a mouse, with the metrics lifted from `mobile/components/filters` (the 11px/0.6-tracking uppercase heading, the 13px/500 fully rounded pill) and the colours from the palette both clients share, so neither can drift. What differs is the ground: these sit on the rail's tinted green card, where `fg.muted` is too close to the tint to read and `bg.subtle` is a grey patch — so ink is the tint's own `app.green.secondary` (tuned to 4.9:1 on `app.green.primary`), a pill rests on `bg.panel`, and washes are that ink at low alpha. A pill is state, so it is fully round; `RailActionButton`/`RailIconButton` are the other half of that rule, squared at `shared/filters`' own `PRESET_BUTTON_RADIUS` so the two clients cannot drift, and never rendering as selected. `RailSegmented` is for an exclusive choice where one option is always on: position says which, so colour is free to say whether the filter is *doing* anything — which is how "Any" gets a neutral thumb (`app.cardBackground`, not `bg.panel`, or it would vanish into the track in dark mode) while Interested and Going take the orange and green trios
...
- [x] `Showtimes/SeatAvailabilityBadge.tsx` — The busyness badge on a feed row, off the payload the feed already carries. The calmest level draws nothing: the badge exists to say "hurry", and "Nearly empty" is the absence of that
...
- [x] `TopBar.tsx` — The *secondary* bar a page can fix under the navigation (the film page's day filters, the friends page's search) — not the site nav, which is `TopNavBar`. Full width now that there is no sidebar to start after; all it clears is the notice banner and `TOP_NAV_HEIGHT`, neither of which changes width
...
- [x] `sheets/sheet-warm-up.ts` — Why every sheet now starts to rise in one frame. A sheet gorhom still has mounted animates from inside `present()`'s own `requestAnimationFrame`; one it has to build first goes present → `setState({mount})` → a commit for the portal → the portal's effect → another commit → mount and layout, and only then moves. Measured at ~325ms with the JS thread completely idle. So a sheet that would be built on its first open is instead presented and closed once at startup, invisible (`opacity: 0`, no backdrop) and instantaneous, and is a mounted closed sheet from then on. It is also what replaced `dismissWhenClosed`: @gorhom/portal never reorders a portal, so z-order is fixed by first-registration order, and rebuilding a sheet on every open was the only way to get it in front of the one it opens from. Warming registers them in a known order instead — one queue, one at a time, because whoever warms later draws in front, and React runs a child's effects before its parent's, so a sheet rendered inside another sheet's component must never be warmed
