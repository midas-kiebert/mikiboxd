 ...
 * invalidated friends query refetches. That matters most when several friends
 * are flipped in a row: each write's refetch can land while the next friend's
 * write is still queued behind it on the server (the backend serializes them
 * per owner), and returns that friend's old value.
 *
 * Shared by the app (`FriendCard`, `FriendAgendaOptions`) and the website
 * (`Friends/friend-controls`); each passes its own way of reporting a failure.
 */
...
export function useFriendStatusSharing(
  friendId: string,
  serverValue: boolean,
  /** Told when a write fails; the control has already fallen back by then. */
  onError?: () => void
): FriendStatusSharing {
  ...
  // Read through a ref so an inline callback does not rebuild `write`.
