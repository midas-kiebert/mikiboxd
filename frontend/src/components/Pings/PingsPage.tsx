...
import { Center, Flex, Heading, Stack, Text, Button, Box, Spinner } from "@chakra-ui/react";
import { Link as RouterLink } from "@tanstack/react-router";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { MeService, type ShowtimePingPublic, type ShowtimeLoggedIn } from "shared";
...
const formatPingSenderSummary = (senders: GroupedPingCard["senders"]) => {
    const names = [...new Set(senders.map((sender) => sender.display_name?.trim() || "Friend"))];
    ...
    const groupedPings = useMemo(() => {
        ...
        if (sortMode === "showtime-date") {
            return groupedCards.sort((left, right) => {
                const leftShowtimeTime = DateTime.fromISO(left.showtime.datetime).toMillis();
                const rightShowtimeTime = DateTime.fromISO(right.showtime.datetime).toMillis();
        ...
        return groupedCards.sort((left, right) =>
            right.latestPingCreatedAtMs - left.latestPingCreatedAtMs
        );
                        ...
                        >
                            ...
                            >
                                <Box>
                                    ...
                                    <Text fontSize="xs" color="gray.500">
                                        Latest invite: {DateTime.fromISO(ping.latestPingCreatedAt).toFormat("ccc, LLL d • HH:mm")}
                                    </Text>
                                </Box>
                                <Button
                                    size="xs"
