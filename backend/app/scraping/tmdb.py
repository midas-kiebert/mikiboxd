...
def find_tmdb_id(
    ...
    if search_results:
        ...
        potential_movies_filtered = [m for m in potential_movies if m['id'] in search_ids]
    ...
    logger.debug(f"No direct match found for '{title_query}' with director '{director_name}' and actor '{actor_name}'. Fuzzy matching...")

    # Score by fuzzy match
    scored = [
        (max(fuzz.token_set_ratio(title_query.lower(), m['title'].lower()),
             fuzz.token_set_ratio(title_query.lower(), m['original_title'].lower())), m)
        for m in potential_movies
    ]
    best_score, best = max(scored, key=lambda x: x[0])

    if best_score < FUZZ_THRESHOLD:
        logger.debug(f"Best match score ({best['title']}) for '{title_query}' is below threshold: {best_score}.")
        return None
...
if __name__ == '__main__':
    # Example
    tmdb_id = find_tmdb_id("the adventures of tintin",
