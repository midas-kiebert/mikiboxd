 ...
 * "fr", "ja"). The filter `LANGUAGE_LABEL` maps in the clients' filter code only
 * cover the subtitle-relevant nl/en pair, so this fuller map is used wherever we
 * want to show the film's main spoken language.
 *
 * Shared rather than per-client: a film listed as "Nederlands" on one and
 * "Dutch" on the other is the same drift the rest of `shared/` exists to stop.
 */
