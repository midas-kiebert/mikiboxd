...
from app.services import unidentified_listings as unidentified_listings_service
...
async def _run_single_cinema_scraper(
    ...
    try:
        ...
        if cinema_id is not None:
            ...
            unidentified_listings_service.consume_unidentified_listings(cinema_id)
        ...
        unidentified_listings = (
            unidentified_listings_service.consume_unidentified_listings(cinema_id)
        )
        unidentified_matches: list[unidentified_listings_service.UnidentifiedListingMatch] = []
        if unidentified_listings:
            with get_db_context() as db_session:
                unidentified_matches = (
                    unidentified_listings_service.find_cineville_matches(
                        session=db_session,
                        cinema_id=cinema_id,
        ...
        if unidentified_matches:
            unidentified_listings_service.record_unidentified_listing_matches(
                unidentified_matches
            )
            logger.warning(
