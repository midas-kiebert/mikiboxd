...
usage() {
  cat <<'EOF'
Build an Android artifact for the mobile app.

Usage:
  ./scripts/build-android.sh [apk|aab] [debug|release]

Examples:
  ./scripts/build-android.sh apk debug
  ./scripts/build-android.sh apk release
  ./scripts/build-android.sh aab release
...
Notes:
  ...
  For a Play-uploadable release build, configure the release keystore with:
    MIKINO_UPLOAD_STORE_FILE
    MIKINO_UPLOAD_STORE_PASSWORD
    MIKINO_UPLOAD_KEY_ALIAS
    MIKINO_UPLOAD_KEY_PASSWORD
EOF
}
...
if [[ "$ARTIFACT" != "apk" && "$ARTIFACT" != "aab" ]]; then
  echo "Invalid artifact type: $ARTIFACT"
  echo "Use apk or aab."
  exit 1
fi
...
OUT_DIR="$MOBILE_DIR/dist/$ARTIFACT"

if [[ "$ARTIFACT" == "apk" && "$BUILD_TYPE" == "release" ]]; then
...
elif [[ "$ARTIFACT" == "apk" && "$BUILD_TYPE" == "debug" ]]; then
  ./gradlew assembleDebug
  ARTIFACT_CANDIDATES=(
    "$ANDROID_DIR/app/build/outputs/apk/debug/app-debug.apk"
  )
elif [[ "$ARTIFACT" == "aab" && "$BUILD_TYPE" == "release" ]]; then
  ...
  ./gradlew bundleDebug
  ARTIFACT_CANDIDATES=(
    "$ANDROID_DIR/app/build/outputs/bundle/debug/app-debug.aab"
  )
fi
...
if [[ -z "$ARTIFACT_PATH" ]]; then
  echo "${ARTIFACT^^} not found after Gradle build."
  exit 1
fi

mkdir -p "$OUT_DIR"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
OUT_PATH="$OUT_DIR/mobile-${BUILD_TYPE}-${TIMESTAMP}.${ARTIFACT}"
cp "$ARTIFACT_PATH" "$OUT_PATH"

echo "${ARTIFACT^^} built: $ARTIFACT_PATH"
